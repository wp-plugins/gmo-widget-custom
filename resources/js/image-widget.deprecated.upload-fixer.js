jQuery(document).ready(function() {
    ...
    jQuery('form#image-form').submit(function(){
        ...
        // _wp_http_referer only contains the widget_custom if the
        // previous action was pressing the add image link in an Widget Custom
        // https://developer.mozilla.org/en/Core_JavaScript_1.5_Reference/Objects/String/indexOf
        if( wp_ref.indexOf('widget_custom') != -1 ) {
            var parsed_url = parse_url(wp_ref);
            var nw_action_url = jQuery('form#image-form').attr('action');

            // make sure the widget_custom is not part of the form action url
            // so we will add it to fix the context
            if( nw_action_url.indexOf('widget_custom') == -1 ) {
...
/* /wp-admin/media-upload.php?type=image&widget_id=widget_custom-11& */